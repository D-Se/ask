lhs <- \(f, n = 1) .Call(c_lhs, f, parent.frame(n), PACKAGE = "ask")
rhs <- \(f, n = 1) .Call(c_rhs, f, parent.frame(n), PACKAGE = "ask")
cases <- \(x) .Call(c_cases, x, PACKAGE = "ask")
...
`?` <- function(., query = NULL) {
	cases(.) |> switch(
		...
		data.table::fifelse(., lhs(query, 2), rhs(query, 2)), # large ifelse
		{
			if (!is.null((y <- substitute(query)))) {
				length(y) |> switch(
					as.character(y) |> switch(
						...
						na = is.na(.), sym = is.symbol(.), nan = is.nan(.),
						dbl = is.double(.),
