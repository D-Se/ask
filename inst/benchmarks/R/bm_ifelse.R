vec_data <- function(n, type, recycle = FALSE, ...) {
	stopifnot(n > 1)
	box::use(stringi[stri_rand_strings])
	x <- sample(c(T, F, NA), n, replace = TRUE, prob = c(.4, .4, .2))
...
define_times <- function(size, robust = 1) {
	window = c(2e1, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, Inf)
	ind <- min(which(size <= window))
	#repeats = c(1e6, 1e6, 1e6, 1e6, 1e5, 1e4, 1e3, 1e2, 1e1, 1e1)
...
vec_dt <- function(data, times = FALSE, threads = FALSE, batch = FALSE, ...) {
...
vec_kit <- function(data, times = FALSE, threads = FALSE, batch = FALSE, ...) {
...
vec_base <- function(data, times = FALSE, threads = FALSE, batch = FALSE, ...) {
	if(!times) define_times(length(data[["x"]]), ...)
...
vec_dplyr <- function(data, times = FALSE, threads = FALSE, batch = FALSE, ...) {
	if(!times) define_times(length(data[["x"]]), ...)
